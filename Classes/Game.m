...
//	NSLog(@"Game::step");
    ...
    // Update the bird's X position
	bird_pos.x += bird_vel.x * dt;
	
    
    // Make sure the bird is facing the right direction
    ...
    //prevent the bird from exiting the sides of the screen
    ...
    // Update the Y vel and pos of the bird
	bird_vel.y += bird_acc.y * dt;
	bird_pos.y += bird_vel.y * dt;
	
    // Doin some weird bonus shit
	AtlasSprite *bonus = (AtlasSprite*)[spriteManager getChildByTag:kBonusStartTag+currentBonusType];
	if(bonus.visible) {
	...
	if(bird_vel.y < 0) {
		...
		BitmapFontAtlas *scoreLabel = (BitmapFontAtlas*)[self getChildByTag:kScoreLabel];
		[scoreLabel setString:scoreStr];
	} else if(bird_acc.y == 0) {
			AtlasSprite *platform = (AtlasSprite*)[spriteManager getChildByTag:landedPlatform];
...
- (void)land:(int) platformTag{
	bird_vel.y = 0;//350.0f + fabsf(bird_vel.x);
    bird_acc.y = 0;
    landedPlatform = platformTag;
}
...
-(void) ccTouchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (int i=0; i<[allTouches count];i++) {
        CGPoint position = [(UITouch*)[allTouches objectAtIndex:i] locationInView:[[Director sharedDirector]openGLView]];
        if (position.y < 240 && bird_acc.y == 0)
        {
            [self jump];
